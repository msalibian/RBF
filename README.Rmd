...
as above. Again, we use 100 runs of 5-fold cross-validation, and
...
```
The boxplots of the mean squared prediction 
...
prediction errors. Note that the we expect a robust method to 
perform slightly worse than the classical one when no model
deviations occur. The boxplots below show that this loss 
in prediction accuracy is very small. 
```{r boxplot.clean.predictions} 
