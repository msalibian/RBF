...
robust backfitting estimator for additive models implemented
in the package `RBF`. This estimator was originally proposed in 
Boente G, Martinez A, Salibian-Barrera M. (2017). 

We will use two data sets. With the first one we illustrate
the robustness of the robust backfitting estimators 
when a small 
proportion of very large outliers are present in the training
set, and compare them with those obtained with the standard backfitting 
algorithm. With the second example, we show how these robust estimators
...
summary(robust.fit)
```
OJO! This function returns the estimation of the intercept, the five-number summary and the mean of the residuals, the estimate of the residual standard error $\widehat{\sigma}$ and a robust version of the R-squared computed as 
...
where $\widehat{a}$ is a robust estimator of $a$ under the location regression model $Y=a+\epsilon$, that is, when no additive regression model is assumed, $R_i=Y_i-\widehat{\mu}-\sum_{j=1}^p \widehat{g}_j(X_{ij})$ and $\rho$ is the loss function in the `type` argument.
...
to obtain predicted values at the point given by the average of 
...
partial residuals plots, below we plot   
the robust estimators for each additive component
when computed with and without
added outliers. 
In green and dashed lines the robust estimator computed with the original data set, and in blue and solid lines the robust estimator computed with the contaminated data set.
...
It is easy to see that the fits obtained with the classical
backfitting estimator when the training set contains outliers
are dramatically different from the ones obtained with the original
data set. We use the package `gam` to compute the 
standard backfitting algorithm (based on local kernel regression smoothers). OJO! Bandwidths selected to compute these estimates are again proportional to the standard deviation but a little larger than before in order to avoid badly conditioned matrices. Specifically we set $h_j=(3/4)\widehat{\sigma}_j$.
```{r gam, warning=FALSE}
library(gam)
...
```
In the plots below, the estimates calculated with the original 
